...
def test_multi_rule_selection():
    ...
    try:
        ...
        # 验证结果
        member_count = len(selected[selected['是否团员'] == '是'])
        non_member_count = len(selected[selected['是否团员'] == '否'])
        ...
        print("\n测试案例2：复杂组合条件")
        rules = [
            {'count': 2, 'gender': '男', 'is_class_leader': '是'},
            {'count': 2, 'gender': '女', 'is_member': '是'},
            {'count': 3, 'is_class_leader': '否', 'is_member': '否'}
        ]
        ...
        rules = [
            {'count': 10, 'gender': '男'},
            {'count': 5, 'is_class_leader': '否'}
        ]
        
        result = selector.select_by_rules(rules)
        selected = result['selected_students']
        unique_count = len(selected['学号'].unique())
        print(f"✓ 去重功能正常，抽取{len(selected)}人，其中唯一学生{unique_count}人")
